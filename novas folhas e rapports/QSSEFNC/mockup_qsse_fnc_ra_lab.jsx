...
const eventRows = [
    {
        ...
        agency: "RL GT",
        chantier: "RAN63C",
        title: "Liste FNC T6 - réseaux / voirie",
    ...
    },
    {
        ...
        agency: "AUV EHTP",
        chantier: "RAP22K",
        title: "Absence de contrôle sur tranchées AEP",
        family: "Absence de contrôle",
        status: "Action en cours",
        cost: 25000,
        owner: "Responsable travaux",
        action: "Plan contrôle à formaliser",
        backlog: false,
        tone: "red",
    },
    {
        ...
        agency: "RL TU",
        chantier: "RAQ18B",
        title: "Départ de boue vers avaloir après pluie",
        family: "Environnement / eaux",
        status: "Mesure immédiate faite",
        cost: 1200,
        owner: "QSE chantier",
    ...
    },
    {
        ...
        agency: "AUV",
        chantier: "RAP56C",
        title: "Pré-check enrobés avant intervention de nuit",
        family: "Bonne pratique / contrôle",
        status: "À diffuser",
        cost: 0,
        owner: "Labo / travaux",
        action: "Transformer en support 1/4h qualité",
        backlog: false,
        tone: "blue",
    },
    {
        ...
        agency: "AIN",
        chantier: "RAP99A",
        title: "Risque récurrent de confusion de plans indice B/C",
        family: "Remontée d'info / prévention",
        status: "À analyser",
        cost: 0,
        owner: "Méthodes / travaux",
        action: "Créer alerte prévention documentaire",
        backlog: false,
        tone: "slate",
    },
    {
    ...
    },
    {
        ...
        agency: "LHL",
        chantier: "RAQ31D",
        title: "Stockage provisoire matériaux à clarifier",
        family: "Environnement / stockage",
        status: "Action en cours",
        cost: 0,
        owner: "Chef chantier",
        action: "Définir zone balisée et photo preuve",
        backlog: false,
        tone: "green",
    },
    {
        ...
        agency: "RL GT",
        chantier: "RAQ40F",
        title: "Planche photo autocontrôle avant remblaiement",
        family: "Bonne pratique / traçabilité",
        status: "Validée",
        cost: 0,
        owner: "Qualité RA",
...
const actions = [
    {
        ...
        risk: "Haut",
        register: "FNC",
        detail: "Distinguer date du fait, date de remontée et date de saisie pour éviter une fausse dérive 2026.",
    },
    {
        title: "Créer typologie causes RaLab",
        pilot: "RST / Qualité",
        due: "15/06/2026",
        status: "À faire",
        risk: "Moyen",
    ...
    },
    {
        ...
        risk: "Moyen",
        register: "FAE",
        detail: "Séparer incident environnement, action environnementale et simple observation terrain.",
    },
    {
        ...
        risk: "Faible",
        register: "BP",
        detail: "Créer un circuit validation puis diffusion 1/4h qualité, environnement ou sécurité.",
    },
    {
        title: "Trier les remontées d'infos utiles",
        pilot: "Méthodes / RST",
        due: "15/07/2026",
        status: "À cadrer",
        risk: "Moyen",
        register: "INFO",
        detail: "Permettre à une remontée d'info de devenir FNC, FAE, BP ou action préventive.",
    },
];
...
const registerConfig = {
    ALL: {
        ...
        description: "Vue consolidée QSSE avec FNC, FAE, BP et remontées d'infos.",
    },
    FNC: {
        label: "Registre FNC",
        description: "Non-conformités, coûts, causes, traitements et actions correctives.",
    },
    FAE: {
        label: "Registre FAE",
        description: "Fiches d'action / événement environnement, mesures immédiates et suivi QSE.",
    },
    BP: {
        label: "Registre BP",
    ...
    INFO: {
        label: "Remontées d'infos",
        description: "Signaux faibles, alertes terrain, idées d'amélioration et prévention.",
    },
};
...
function MonthlyBars({ cleanView, registerFilter }) {
    const values = monthlyData.map((item) => {
        if (registerFilter === "FAE") {
            return item.fae;
        }
        if (registerFilter === "BP") {
            return item.bp;
        }
        if (registerFilter === "INFO") {
            return item.info;
        }
        ...
        <aside className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
                <div className="rounded-2xl border border-amber-100 bg-amber-50 p-3">
                    ...
                    <p className="mt-1 text-xs text-amber-900">79 FNC saisies en 2026 mais rattachées à des faits 2025.</p>
                </div>
                <div className="rounded-2xl border border-red-100 bg-red-50 p-3">
                    <div className="text-sm font-bold text-red-950">Actions correctives manquantes</div>
                    <p className="mt-1 text-xs text-red-900">FNC à verrouiller avant clôture si cause ou action absente.</p>
                </div>
                <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-3">
                    <div className="text-sm font-bold text-emerald-950">FAE à distinguer</div>
                    <p className="mt-1 text-xs text-emerald-900">Séparer incident environnement, action environnementale et simple observation.</p>
                </div>
                <div className="rounded-2xl border border-blue-100 bg-blue-50 p-3">
                    <div className="text-sm font-bold text-blue-950">BP à diffuser</div>
                    <p className="mt-1 text-xs text-blue-900">Les bonnes pratiques doivent pouvoir devenir REX ou support 1/4h.</p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <div className="text-sm font-bold text-slate-950">Remontées d'infos à trier</div>
...
function DashboardView({ cleanView, dateMode, registerFilter }) {
    const stats = useMemo(() => {
        ...
        return {
            ...
            cost: visibleFnc.reduce((sum, row) => sum + row.cost, 0) + eventRows.filter((row) => row.register === "FAE").reduce((sum, row) => sum + row.cost, 0),
            backlog: eventRows.filter((row) => row.backlog).length,
        };
    }, [cleanView]);
            ...
            <aside className="col-span-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 space-y-3">
                    <div className="rounded-2xl bg-blue-50 p-4 text-sm text-blue-950">
                        ...
                        <p className="mt-1">Une page synthétique pour direction, qualité, environnement et travaux.</p>
                    </div>
                    ...
                    <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">
                        <div className="font-black">Support 1/4h qualité</div>
                        <p className="mt-1">Version courte issue des FNC, BP ou remontées d'infos.</p>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">
                        <div className="font-black">Plan d'actions</div>
                        <p className="mt-1">Actions curatives, correctives, préventives et diffusion.</p>
                    </div>
                </div>
            </aside>
        </div>
...
export default function MockupQsseFncRaLab() {
                ...
                <header className="mb-5 rounded-3xl bg-blue-950 p-5 text-white shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-2 max-w-3xl text-sm leading-6 text-blue-100">Mockup d'un module capable de gérer les non-conformités, actions ou événements environnement, bonnes pratiques, remontées d'infos, imports historiques et REX.</p>
                        </div>
                    ...
                    <div className="col-span-6 rounded-3xl border border-slate-200 bg-white p-3 shadow-sm">
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            >
                                ...
                                <option value="INFO">Remontées d'infos</option>
                            </select>
                            <select
                                value={dateMode}
