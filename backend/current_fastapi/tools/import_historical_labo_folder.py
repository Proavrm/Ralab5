...
Legacy alias — préférer import_historical_labo_folder_v2.py (grouped import V2).
"""
...
def main() -> None:
    print(
        "[DEPRECATED] import_historical_labo_folder.py — use import_historical_labo_folder_v2.py",
        file=sys.stderr,
    )
    parser = argparse.ArgumentParser(description="Run historical lab Excel import V2 (legacy entrypoint).")
    parser.add_argument("folder_path", help="Unzipped folder containing Excel files")
    parser.add_argument("--target-db", dest="target_db", default=str(get_db_path()))
    parser.add_argument("--affaires-db", dest="affaires_db", default=str(ROOT_DIR / "data" / "affaires.db"))
