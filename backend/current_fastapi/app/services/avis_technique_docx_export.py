...
def _render_meta(doc: Any, content: dict[str, Any], instance: dict[str, Any]) -> None:
    ...
    if not merged:
        merged = {
            ...
            "title": instance.get("titre") or "",
            "author": instance.get("auteur") or "",
            "status": instance.get("statut") or "",
        }
    _render_kv_table(doc, {"rows": [{"key": k, "value": v} for k, v in merged.items()]})
...
def _is_annex_section(section: dict[str, Any]) -> bool:
    ...
    return sid.startswith("a") or "annexe" in title.lower()
...
def build_avis_docx_bytes(instance: dict[str, Any], template: dict[str, Any] | None = None) -> bytes:
    ...
    # Prefer reference Word as style donor (styles only — body cleared)
    ...
    if style_file and style_file.exists():
        ...
        # Clear body while keeping styles
    ...
    # Couverture
    _add_heading(doc, str(title), level=0 if False else 1)
    meta_rows = []
    if ref:
        meta_rows.append({"key": "Référence", "value": ref})
    if instance.get("auteur"):
        meta_rows.append({"key": "Auteur", "value": instance["auteur"]})
    if instance.get("statut"):
        meta_rows.append({"key": "Statut", "value": instance["statut"]})
    meta = instance.get("meta") if isinstance(instance.get("meta"), dict) else {}
    for key in ("document_date", "destinataire", "lieu", "status"):
        if meta.get(key):
            meta_rows.append({"key": key, "value": meta[key]})
    if meta_rows:
        _render_kv_table(doc, {"rows": meta_rows})
    ...
    for section in sections:
        ...
        if section_id == "cover":
            # already rendered via meta
            slot = (contents.get("cover") or {}).get("meta")
            if isinstance(slot, dict):
                render_block(doc, "meta_document", slot.get("content") or {}, instance)
            continue
        ...
        for label, bt, content in pending:
            if label and label not in {"Texte", "Commentaire", "Introduction", "Fiches de calcul"}:
