...
class AffairesRstRepository:
    ...
    def _ensure_schema(self) -> None:
        with self._connect() as conn:
            existing = {row[1] for row in conn.execute("PRAGMA table_info(affaires_rst)").fetchall()}
            migrations = {
                "site": "ALTER TABLE affaires_rst ADD COLUMN site TEXT NOT NULL DEFAULT ''",
                "numero_etude": "ALTER TABLE affaires_rst ADD COLUMN numero_etude TEXT NOT NULL DEFAULT ''",
                "filiale": "ALTER TABLE affaires_rst ADD COLUMN filiale TEXT NOT NULL DEFAULT ''",
            }
    ...
    def all(self, statut=None, titulaire=None, search=None, a_revoir=None) -> list[AffaireRstRecord]:
        ...
        if search:
            sql += """
                AND (
                    a.reference LIKE ? OR a.client LIKE ? OR a.chantier LIKE ? OR a.site LIKE ?
                    OR a.affaire_nge LIKE ? OR a.numero_etude LIKE ? OR a.responsable LIKE ? OR a.filiale LIKE ?
                )
            """
            like = f"%{search}%"
    ...
    def find_by_affaire_nge(self, affaire_nge: str) -> AffaireRstRecord | None:
        value = (affaire_nge or "").strip()
        if not value:
            return None
        with self._connect() as conn:
            row = conn.execute(
    ...
    def find_by_numero_etude(self, numero_etude: str) -> AffaireRstRecord | None:
        value = (numero_etude or "").strip()
        if not value:
            return None
        with self._connect() as conn:
            row = conn.execute(
    ...
    def distinct_values(self, column: str) -> list[str]:
        allowed = {"statut", "titulaire", "region", "filiale"}
    ...
    def add(self, record: AffaireRstRecord) -> AffaireRstRecord:
        ...
        try:
            ...
            annee, region, numero = datetime.now().year, "RA", 0
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO affaires_rst
                (
                    reference, annee, region, numero, client, titulaire, chantier, site,
                    numero_etude, affaire_nge, filiale, date_ouverture, date_cloture,
                    statut, responsable, created_at, updated_at
                )
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    ...
                    record.affaire_nge,
                    record.filiale,
                    self._fmt(record.date_ouverture),
                    self._fmt(record.date_cloture),
                    record.statut,
                    record.responsable,
                    now,
                    now,
    ...
    def update(self, uid: int, fields: dict) -> AffaireRstRecord:
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        fields = dict(fields)
        for key in ("date_ouverture", "date_cloture"):
    ...
    def _row(self, row: sqlite3.Row) -> AffaireRstRecord:
        ...
        return AffaireRstRecord(
            ...
            titulaire=row["titulaire"] or "",
            chantier=row["chantier"] or "",
            site=row["site"] or "",
            numero_etude=row["numero_etude"] or "",
            affaire_nge=row["affaire_nge"] or "",
            filiale=row["filiale"] or "",
