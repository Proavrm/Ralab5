...
def _normalize_intervention_link_id(intervention_id: Optional[int], intervention_reelle_id: Optional[int]):
    raw = intervention_id if intervention_id else intervention_reelle_id
...
def _get_prelevement_row(conn: sqlite3.Connection, prelevement_id: Optional[int]):
    if not prelevement_id:
        return None
    return conn.execute(
        "SELECT id, demande_id, intervention_id, date_prelevement, zone FROM prelevements WHERE id = ?",
        (prelevement_id,),
    ).fetchone()


def _get_intervention_reelle_row(conn: sqlite3.Connection, intervention_reelle_id: Optional[int]):
    return _get_intervention_row(conn, intervention_reelle_id)
...
def _resolve_echantillon_links(
    ...
    linked_intervention_id = _normalize_intervention_link_id(intervention_id, intervention_reelle_id)
    ...
    if linked_prelevement:
        derived_intervention_id = linked_prelevement["intervention_id"]
        if (
            linked_intervention_id
            and derived_intervention_id
            and linked_intervention_id != derived_intervention_id
        ):
            raise HTTPException(400, "Le prélèvement et l'intervention sélectionnés ne correspondent pas")
        linked_intervention_id = derived_intervention_id

    linked_intervention_reelle = _get_intervention_row(conn, linked_intervention_id)
    if linked_intervention_id and not linked_intervention_reelle:
        raise HTTPException(404, f"Intervention #{linked_intervention_id} introuvable")
...
def _compute_result_summary(
    ...
    if code in {"IPI", "IM", "CBRI", "CBR"} or typ in {"ipi", "cbri", "cbr"}:
        value = _first_float(r.get("ipi"), r.get("cbr"), r.get("cbr_95"))
        if value is not None:
            label_code = "IPI" if code == "IPI" else ("CBRi" if code in {"IM", "CBRI"} else "CBR")
            return round(value, 2), "%", f"{label_code} = {value:.2f} %"
        tests = r.get("tests") if isinstance(r.get("tests"), list) else []
        vals = [_first_float(t.get("ipi"), t.get("cbr")) for t in tests if isinstance(t, dict)]
...
def _row(row) -> dict:
    ...
    if data.get("intervention_reelle_reference") and not data.get("intervention_reference"):
        data["intervention_reference"] = data.get("intervention_reelle_reference")
...
def list_echantillons(
    ...
    sql = """
        SELECT
            ...
            i.reference AS intervention_reelle_reference,
            i.type_intervention AS intervention_reelle_type,
        ...
        LEFT JOIN interventions i ON i.id = ech.intervention_id
        WHERE 1=1
    """
    params = []
    with _conn() as conn:
        ...
        if linked_intervention_id is not None:
            sql += " AND ech.intervention_id = ?"
...
def create_echantillon(body: EchantillonCreate):
    ...
    with _conn() as conn:
        ...
        conn.execute(
            ...
            (reference,annee,labo_code,numero,demande_id,prelevement_id,intervention_id,
            ...
            """,
            (
                ref, annee, labo, numero, body.demande_id, linked_prelevement_id, linked_intervention_id,
...
def update_echantillon(uid: int, body: EchantillonUpdate):
    ...
    with _conn() as conn:
        ...
        if "prelevement_id" in fields or "intervention_id" in fields or "intervention_reelle_id" in fields:
            current = conn.execute(
                "SELECT prelevement_id, intervention_id FROM echantillons WHERE id = ?",
                (uid,),
            ).fetchone()
            ...
            requested_intervention_id = fields.get(
                "intervention_id",
                current["intervention_id"] if current else None,
            )
            ...
            fields["intervention_id"] = linked_intervention_id
